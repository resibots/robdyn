...
void Environment::_init(bool add_ground, float _angle)
{
    ...
    if (add_ground)
    {
        ...
        //std::cerr << " q1 " << q1.x() << " " << q1.y() << " " << q1.z() << " " <<  q1.w() << std::endl;
        //std::cerr << " q2 " << q1.x() << " " << q1.y() << " " << q1.z() << " " <<  q1.w() << std::endl;
...
void Environment::_init_with_obstacles(bool add_ground, float floor_angle, std::vector<std::vector<float> > pos_rad)
{
    //create world
    _world_id = dWorldCreate();
    //init gravity
    ...
    if (add_ground)
    {
        ...
        /*_obs_body = dBodyCreate (_world_id);
        _obs_geom = dCreateSphere (_space_id, 1.0); //radius
        dMass m; dMassSetSphere (&m, 0, 1.0); // density; radius
        dBodySetMass (_obs_body, &m);
        dGeomSetBody (_obs_geom, _obs_body);
        // set initial position
        dBodySetPosition(_obs_body, 0.0, 1.5, 0.0);
        _ground_objects.insert(_obs_geom);*/

        obstacle_pos_rad = pos_rad;
        for(size_t i=0; i<obstacle_pos_rad.size();++i)
        {
            _obs_geom = dCreateSphere (_space_id, obstacle_pos_rad[i][3]);
            dGeomSetPosition(_obs_geom, obstacle_pos_rad[i][0], obstacle_pos_rad[i][1], obstacle_pos_rad[i][2]);
            _ground_objects.insert(_obs_geom);
            obstacle_objects.push_back(_obs_geom);
        }
    }
    //contact group1
    _contactgroup = dJointGroupCreate(0);
